...
export class WinScene extends Phaser.Scene {
  ...
  create() {
    ...
    const winnerText = this.add.text(W / 2, 170, `${teamEmoji} ${teamName} WINS! ${teamEmoji}`, {
      fontSize: '42px', color: teamColor, fontStyle: 'bold',
    ...
    this.add.text(W / 2, 230, modeLabels[this.mode] || this.mode, {
    ...
    // ── Rematch Button ──
    this._createButton(W / 2, H - 140, '🔄 QUICK REMATCH', CONFIG.COLORS.GREEN, () => {
      SocketManager.rematch();
    });

    // ── Back to Lobby ──
    this._createButton(W / 2, H - 70, '🏠 Back to Lobby', CONFIG.COLORS.PURPLE, () => {
  ...
  _showScoreSummary(W, H) {
    ...
    // Panel
    this.add.rectangle(W / 2, centerY + 30, 500, 150, 0x111122, 0.8)
      .setStrokeStyle(2, 0x333366);

    this.add.text(W / 2, centerY - 30, '📊 Final Scores', {
      fontSize: '22px', color: '#ffd700', fontStyle: 'bold'
    }).setOrigin(0.5);

    // Red score
    this.add.text(W / 2 - 100, centerY + 10, `🔴 Red: ${scores.red}`, {
      fontSize: '28px', color: '#ff6b6b', fontStyle: 'bold'
    }).setOrigin(0.5);

    // Blue score
    this.add.text(W / 2 + 100, centerY + 10, `🔵 Blue: ${scores.blue}`, {
    ...
    this.add.text(W / 2, centerY + 55, statsText, {
      fontSize: '16px', color: '#888'
    }).setOrigin(0.5);

    // Hint text
    this.add.text(W / 2, centerY + 90, 'Press R for rematch  |  ESC for lobby', {
